...
\chapter{Learning amortized alorithms in machines}
\label{introduction}
...
% and is present in human babies, rats, and even some bird \citep{leslie1982perception,gopnik2001causal,gopnik2004theory,blaisdell2006causal, lagnado2013causal}.
...
%There is a rich literature on formal approaches for defining and performing causal reasoning \citep{pearl2000,spirtes2000causation,dawid07fundamentals,pearl16causal}, that are also popularly used to model causal reasoning in humans \citep{glymour2003learning}. However, human causal inferences often deviate from normative causal inference. There is evidence for Markov violations \citep{rehder2014independence},
%% \footnote{When inferring Z from the structure $X \rightarrow Y \rightarrow Z$ or $X \leftarrow Y \rightarrow Z$, people think that X is relevant even when Y completely mediates the relationship between X and Z}
%failures in explaining away \citep{rehder2017failures}, 
%% \footnote{When inferring Y from the structure $X \rightarrow Y \leftarrow Z$, in the presence of one potential cause (say X) for Y, people fail to proportionately reduce the the probability that Z too is present}
%neglect of relevant variables \citep{fernbach2010neglect}, and failure to integrate over hidden variables \citep{fernbach2013cognitive}. These behaviours result in responses that resemble more associative reasoning than purely normative causal reasoning. Observed behavior patterns are also not consistent and show significant variation depending on mechanisms \citep{lombrozo2010causal}, and experience \citep{krynski2007role}. These empirical findings indicate that causal reasoning in humans is not implemented with an entirely abstract domain-general normative theory.

%We can get some insight into causal inference in humans by considering its origin.  This indicates a potential explanation for the non-normativity of human causal reasoning -- 
...
% Leveraging such underlying statistical structure for causal identification and inference is also an active area of research in machine learning \citep[e.g.][]{heckerman1995learning, bengio2019meta, magliacane2018domain, parascandolo2017learning}.
...
% \jane{Maybe the results can be framed as: what structure does the training set contain? what behaviors should we expect to be optimal, and how can we differentiate? Frame first as: we hypothesize that if agents are indeed acquiring causal understanding, they should behave like X, and perform better than baseline agent Y. Call attention to the differences in the task distributions from experiment to experiment, and highlight the fact that it's only these differences that are inducing the different levels of performance and behaviors.}


% The purpose of this work is notto propose new algorithmic solutions to causal inference per se. Rather, we believe that by examining the emergence of causal learning from simpler learning mechanims, with a the meta-learning perspective, we can build links with human causal reasoning and shed light on how agents could better learn in more ecologically valid scenarios. Indeed, in real-world environments, humans must operate under both time and resource constraints, dealing with uncertainty in model class as well as nonstationarity. 
...
% This also provides a basis for modelling the domain/function specificity \citep{krynski2007role, lombrozo2010causal} seen in humans. 
...
\section{Task Setup and Agent Architecture}
\label{sec:task}
...
\subsection*{Causal Graphs, Observations, and Actions}
...
\textbf{Active vs Random conditions.}
...
\textbf{Agent Architecture and Training.} For the agent, we use a long short-term memory (LSTM) network \citep{hochreiter97long}. At each time-step $t$, the LSTM receives a concatenated vector containing $[o_{t}, a_{t-1}, r_{t-1}]$ as input.
%, where $o_{t}$ is the observation, $a_{t-1}$ is the previous action, $r_{t-1}$ the previous reward and $m_t$ indicates the external intervention (relevant only in the last step of the information phase going into the quiz phase). 
The LSTM outputs are a softmax over the number of actions and a scalar baseline. Learning is done by \emph{asynchronous advantage actor-critic} \citep{mnih2016}. See Supplementary Material for further details. In all experiments, the agent is tested with the learning rate set to zero using a held-out test set as discussed above.
...
\subsection{Experiment 1: Observational Environments}
\label{sec:expt1}
...
\subsection{Experiment 2: Interventional Environments}
\label{sec:expt2}
...
% An example of this would arise when the intervened upon node has no parents, thus rendering causal reasoning equivalent to associative reasoning. 
...
\section{Formalism for Memory-based Meta-learning \label{sec:meta-formalism}}
...
\section{Formalisms for causal inference}

\subsection{Causal Bayes Nets}
...
\subsection{An intuitive example of cause-effect reasoning in a \CBN} 
...
\section{RL Baselines}

% \begin{wrapfigure}{l}{0.2\textwidth}
% \centering
% \includegraphics[width=0.2\textwidth]{figures/reward_dist.pdf} 
...
\subsection{Experiment 3: Counterfactual Setting}
\label{sec:expt3}
